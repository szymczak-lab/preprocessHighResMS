...
#'
#' Extract information for annotation from SummarizedExperiment object. For each
#' feature, the median mz value across all samples will be used as measured mz
#' value and the sum of all intensities for the intensity value.
#'
#' @param se \code{\link[SummarizedExperiment]{RangedSummarizedExperiment-class}}
#' object
#'
#' @return
#' data.frame with information about measured features with columns
#' id (= unique identifier), mz (= measured mz value), intensity (= measured
#' intensity), no.peak.detected (= number of detected peaks).
#'
...
prepare_data_for_annotation <- function(se) {
    ...
    dat = matrix(nrow = nrow(se),
                 ncol = 3,
                 dimnames = list(NULL,
                                 c("mz", "intensity", "no.peaks.detected")))
    for (i in 1:nrow(se)) {
...
find_hits <- function(info.features, dat, ppm) {
    ...
    ## find overlaps
    hits.m = matrix(0, nrow = nrow(dat), ncol = nrow(info.features),
                    dimnames = list(dat$id,
                                    info.features$id))

    for (i in 1:nrow(hits.m)) {
        deltaM = ppm * dat$mz[i] * 1e-06
        ind = which(abs(info.features$m.z - dat$mz[i]) <= deltaM)
        if (length(ind) > 0) hits.m[i, ind] = 1
    }
    ...
    info.features.iso = info.features[which(info.features$isotope > 1 &
                                                info.features$id %in%
                                                colnames(hits.m)), ]
    # if (nrow(info.features.iso) > 0) {
    #     orphans = NULL
    #     for (i in 1:nrow(info.features.iso)) {
...
compute_prior_prob <- function(hits.m,
    ...
    prior.m = matrix(0, nrow = nrow(hits.m),
                     ncol = ncol(hits.m),
    ...
    if (is.null(pk)) {
        ...
        stop(paste("length of pk needs to be equal to number of columns of",
                   "hits.m (+1 if ppm.unknown is specified)!"))
    }

#    sigma = (ppm * info.features$m.z) / (2 * 10^6)
    sigma = (ppm * dat$mz) / (2 * 10^6)
    for (i in 1:nrow(prior.m)) {

        d = exp((-0.5 * 1/(sigma[i]^2)) * (dat$mz[i] - info.features$m.z)^2)
    ...
    if (!is.null(ppm.unknown)) {
        ...
        if (sum.unknown == 0) {
            print("column unknown removed since none of the probabilities is > 0!")
            prior.m =
...
#' monoisotopic isotopes and adducts of the same chemical formula and 0 otherwise.
...
generate_connectivity_matrix <- function(info.features,
    ...
    conn.m = matrix(0, nrow = nrow(info.features),
...
compute_posterior_prob <- function(prior.prob,
    ...
    for (it in 1:no.its) {
        ...
        for (thism in ordine) {
            ...
            p.add <- pot.add - colSums(add.m[sampcomp[thism], , drop = FALSE])
            p.add <- (p.add + delta.add)/sum(p.add + delta.add)
            if (use.add) po = po * p.add

            p.bio <- pot.bio - colSums(bio.m[sampcomp[thism], , drop = FALSE])
            p.bio <- (p.bio + delta.bio)/sum(p.bio + delta.bio)
            if (use.bio) po = po * p.bio

            p.iso <- pot.iso - colSums(iso.m[sampcomp[thism], , drop = FALSE])
            p.iso <- (p.iso + delta.iso)/sum(p.iso + delta.iso)
            if (use.iso) po = po * p.iso
            ...
            if (oldval != sampcomp[thism]) {
                ...
                pot.add <- pot.add - add.m[, oldval] + add.m[, sampcomp[thism]]
                pot.iso <- pot.iso - iso.m[, oldval] + iso.m[, sampcomp[thism]]
            }
        }
...
#        }
    ...
    post.prob <- t(apply(allsampcomp, 2, compute_post, burn = burn,
                         no.its = no.its, Nc = ncol(prior.prob)))
...
assign_features <- function(post.prob,
    ...
    info.assigned = t(sapply(1:nrow(post.prob), function(i) {
        ind = which.max(post.prob[i, ])
        return(c(f.meas = rownames(post.prob)[i],
    ...
    if (any(tab > 1)) {
        ...
        print(paste("selecting unique measured feature for", length(f.theo.mult),
...
#'
#' @param info.assigned data.frame with information about assignment of measured
#' and theoretical features (e.g. as generated by the function
...
summarize_metabolites <- function(info.assigned,
    ...
    se.met = SummarizedExperiment(assays = list(intensity = int.met,
...
multsample <- function(P) {
    sample(1:length(P), size = 1, replace = TRUE, prob = P)
}
